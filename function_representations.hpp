...
const int N = 2; // Coordinates are 0-based, so {0, 1, 2, ..., N-1}.
...
HP Derive(const HP& p, const vector<int>& coords) {
	HP q = p;
	for (int coord: coords) q = Derive(q, coord);
	return q;
}
...
HP operator*(const HP& p, const HP& q) {
    HP r;
    for (const auto& pp: p.coef) for (const auto& qq: q.coef) {
        vector<int> exponent = pp.first;
        for (int i = 0; i < N; i++) exponent[i] += qq.first[i];
        r.coef[exponent] += pp.second * qq.second;
    }
    return r;
}
...
void operator+=(AE& f, const AE& g) {
	for (auto comp: g.components) f.add(comp.second);
}

AE operator+(AE f, const AE& g) {
	f += g;
	return f;
}
...
AE MultiplyByX(const H& f, int coord) {
	H g1;
	H g2;
...
AE Derive(const AE& f, const vector<int>& coords) {
	AE g = f;
	for (int coord: coords) g = Derive(g, coord);
	return g;
}

AE MultiplyByPolynomial(const AE& f, const Polynomial& z) {
	AE res;
	for (auto pp: z.coef) {
		AE curr = f;
		for (int i = 0; i < N; i++) {
...
AE operator*(const AE& f, const Polynomial& z) {
	return MultiplyByPolynomial(f, z);
}

AE operator*(const AE& f, const HomogeneousPolynomial& z) {
    Polynomial z2;
    z2.coef = z.coef;
    return f*z2;
}

// Specializing the inverse Laplacian for homogeneous polynomials.
// The polynomial is converted to AE (which is likely not very efficient) and
// then the inverse Laplacian for AE is called.
HomogeneousPolynomial InverseLaplacian(const HomogeneousPolynomial& p) {
    // Conversion from HomogeneousPolynomial to AE.
    H one{.k = 0, .h = 0, .p=F(1)};
    AE z(one);
    z = z * p;

    // Inverse Laplacian for AE
    z = InverseLaplacian(z);

    // Conversion from AE to HomogeneousPolynomial.
    HP x2;
    for (int i = 0; i < N; i++) {
        vector<int> exponent(N, 0);
        exponent[i] = 2;
        x2.coef[exponent] = F(1);
    }
    HP pow_x2;
    pow_x2.coef[vector<int>(N, 0)] = F(1);

    HP res;
    for (int k = p.deg() + 2; k >= 0; k -= 2, pow_x2 = pow_x2 * x2) {
        res += pow_x2 * z.components[{k, p.deg()+2-k}].p;
    }
    return res;
}
